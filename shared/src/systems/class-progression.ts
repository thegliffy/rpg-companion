...
export function classLevelEntry(className: string, level: number): ClassLevelEntry | null {
  ...
  if (!entries) return null;
  // Multiple entries can still share a level, so merge every entry at the highest level <= the
  // character's level rather than picking one. Note this table holds *base class* features only:
  // the 5e-database import originally flattened each class's one SRD subclass into it as extra
  // same-level rows, which meant every Warlock got the Fiend's features and every Rogue the
  // Thief's regardless of what they had chosen. Those rows were removed -- subclass features come
  // from SRD_SUBCLASSES / a custom subclass via subclassFeaturesAt(). Placeholder names like
  // "Otherworldly Patron feature" are genuine class-table entries and deliberately stay.
